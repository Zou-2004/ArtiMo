...
def build(
    ...
    prompt = (
        ...
        "a contact link, change a motion owner, or reorder object actions. Consecutive "
        "robot-contact controls on the same contact link with no intervening plan "
        "`control_release` use one uninterrupted `contact_sequence`, even when the "
        "controlled joint changes. A transition to a different physical contact link "
        "automatically creates a robot release/retreat/reacquire boundary before the "
        "next plan phase; it does not invent or reorder object motion.\n\n"
        ...
        "it must never narrow, subsample, or reorder the full sparse candidate "
        "matrix, skip the lateral-offset batch after all centered distances fail, or "
        ...
        "into an execution, release solver, physics rollout, or video. "
        "The sole exception is a machine-emitted `transit_route_repair` whose "
        "classification is `prior_plan_moved_link_blocks_transit`: use its separate "
        "candidate execution only as input to the bounded transit proposer/solver, "
        "never as rollout input. The placement command normally runs that repair "
        "automatically and emits runnable execution only after the route solver passes. "
        "The instruction to export a complete video when diagnostics fail applies only after the "
        "placement solver has emitted its feasible `execution.json`. After sparse "
        "screening, dense rows are already cost-ordered: the application must stop "
        "the dense batch immediately on its first fully feasible row. Do not relaunch "
        "dense to rank or reproduce additional passing rows.\n\n"
        "Do not fill or tune contact quaternions, Panda model constants, plan-derived "
        "phase/control/joint/target fields, contact-sequence or release boundaries, "
        "forbidden object links, passive-return gains, GPU backend, IK budgets, "
        "sparse/dense budgets, or placement bounds. The application overwrites these "
        "from geometry, plan.json and machine defaults. Supply only control ownership, "
        "stage id, interaction, contact link, link-local contact-point translation, "
        "optional final finger opening, and for a "
        "physical push only the semantic robot tool surface. Choose orientation only "
        "by classifying the five application-generated contact_roll_deg renders. For "
        "every render also fill contact_point_status=valid/adjust from the isolated "
        "views. Except for a one-tool physical_push, valid requires the target feature's "
        "cross-section to lie visibly in the open gap between the cyan and magenta contact "
        "pads, with the pads on opposite sides, in at least one isolated view that clearly "
        "exposes the jaw-closing cross-section. Open and review every required view, but an "
        "occluded view is not by itself grounds for adjust when another isolated view clearly "
        "proves this relationship. Feature-pad overlap or same-side pads in the proving view, "
        "or the absence of any proving view, means adjust. This is a visual semantic test; "
        "exact static proxy collision is not required. If it is high, low, or off-feature, "
        "adjust only the "
        "task-local contact-point translation and rerender "
        "the complete five-roll batch. The application never moves that semantic point "
        "automatically. Never "
        "write contact_pose_link.rotation_xyzw or grasp_depth_m. The application resets "
        "agent-supplied grasp depth, searches a shallow-to-deep deterministic depth "
        "lattice, rejects every depth with non-target collision or missing bilateral "
        "finger proximity, and reports the selected depth. During rollout it advances "
        "past acquisition only after both real finger links sustain opposed target-link "
        "contact. Never supply or edit a forbidden/allowed collision-link list.\n\n"
