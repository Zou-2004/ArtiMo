...
```text
asset_name/
  mobility.urdf                           # or pass another URDF with scripts/textured.sh --urdf
...
The URDF provides the joints, link hierarchy, visual origins, mesh filenames or
URDF primitives, and joint limits. Mesh paths are resolved relative to the URDF
location, so the folder names do not have to be exactly `textured_objs/` and
`images/`; they only need to match the paths written in the URDF and the OBJ/MTL
files.
...
animated GLB is generated from the URDF plus its referenced meshes and URDF
primitive visuals (`box`, `cylinder`, `sphere`):

```bash
bash scripts/textured.sh --root /path/to/data/causal_data asset_name
```

For an external asset that has a differently named URDF such as `model.urdf`,
pass the URDF directly. The script copies it to `mobility.urdf`, canonicalizes
link/joint names, and writes the animated textured GLB in the same asset folder:
...
ArtiMo can also start from assets generated by [Articraft](https://github.com/mattzh72/articraft). This makes the whole piepiline start from text and image only! ArtiMo can process Articrraft's URDF outputs directly:
...
This creates `mobility.urdf` and `animated_textured_*.glb` in the same folder.
After that, run the normal ArtiMo pipeline with the materialized folder as
`--asset_root`. This path supports both referenced OBJ meshes and URDF primitive
visuals such as `box`, `cylinder`, and `sphere`.
